    ...
    if (data.response) {
      if (data.response.data) {
        if (data.response.data.status) {
          if (data.response.data.status.status) {
            errorCode = data.response.data.status.status;
            let displayMessage = data.response.data.status.message;
            if (displayMessage == null) {
              displayMessage = "Oops! Something went wrong.";
            }
            if (errorCode == 400) {
...
export const sendOTPToPhoneNumber = async (accountId, number, navigation) => {
  ...
  try {
    ...
    console.log(
      "Data in sendOTPToPhoneNumber: " + JSON.stringify(response.data)
    );
    if (response.data.status.status === 200) {
      alert(
        "Send otp success please enter code in your message to verify your phone number"
      );
    }
  } catch (data) {
    if (data) {
