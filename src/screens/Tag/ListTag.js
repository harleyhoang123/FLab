...
function ListTag({navigation}) {
    ...
    const addATag=(tagName,ownerBy)=>{
        addTag(tagName,ownerBy).then(r=>getAllTag().then(v=> setListTag(v.data.items)))
    }
    const updateATag=(tagId,tagName,ownerBy)=>{
        updateTag(tagId,tagName,ownerBy).then(r=>{getAllTag().then(v=> setListTag(v.data.items))})
    }
    const deleteATag=(tagId)=>{
        deleteTag(tagId).then(r=>{getAllTag().then(v=> setListTag(v.data.items))})
    }
        ...
        <View >
            ...
            <View style={styles.container}>
                ...
                >
                    ...
                    >
                        <View style={styles.modalProfileView}>
                            <Text style={{fontSize: 20, fontWeight: "bold", marginBottom: 20}}>Do you want to delete this tag?</Text>
                            <View style={{alignItems: "flex-end", flexDirection: "row"}}>
                    ...
                    <View style ={styles.containerContent}>
                        ...
                        <View style ={styles.containerSearch}>
                            <TextField text={text} onChangeText={newText => setText(newText)}
                                       ...
                                       multiline={false}
                                       style={{width:400}}/>
                            <Buttons text={"Search"} style={styles.button}/>
                            <Buttons text={"Update"} style={[styles.button,{marginLeft:20}]} onPressTo={()=>setUpdate(true)}  disabled={disable}/>
                            <Buttons text={"Delete"} style={[styles.button,{marginLeft:20}]}  onPressTo={()=>{ setDeleteT(true)}} disabled={disable}/>
                            <Buttons text={"Add Tag"} style={[styles.button,{marginLeft:20}]} onPressTo={()=> setAdd(true)}/>
                        </View>
                    </View>
                    <View style={styles.table}>
