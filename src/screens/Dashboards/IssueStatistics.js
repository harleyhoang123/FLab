...
function IssueStatistics({ navigation }) {
  ...
  useEffect(() => {
    getProjectId().then((v) => {
      getIssueStatistics(v, navigation).then((r) => {
      ...
      <View style={{ width: "75%", margin: 10, alignSelf: "center" }}>
        <View style={{ flexDirection: "row" }}>
          <View style={{ width: "85%", alignSelf: "center" }}>
            <ProgressBar
              progress={calculatePercent(count, totalIssue)}
              color={"blue"}
            />
          </View>
          <Text style={{ alignSelf: "center" }}>
            {" "}
            {calculatePercent(count, totalIssue) * 100}%
          </Text>
        </View>
      </View>
    </View>
          ...
          <View style={{ margin: 20 }}>
            ...
            >
              <Text style={{ fontSize: 18, fontWeight: "bold", margin: 10 }}>
                Issue Statistics: Project (Assignee)
              </Text>
            ...
            <View style={{ flexDirection: "row" }}>
              ...
              <View style={{ width: "75%", margin: 10 }}>
                <View style={{ flexDirection: "row" }}>
                  <View style={{ width: "85%", alignSelf: "center" }}>
                    <ProgressBar
                      progress={totalUnassigned / totalIssue}
                      color={"blue"}
                    />
                  </View>
                  <Text style={{ alignSelf: "center" }}>
                    {" "}
                    {calculatePercent(totalUnassigned, totalIssue) * 100}%
                  </Text>
                </View>
              </View>
            </View>
