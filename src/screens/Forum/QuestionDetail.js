...
function QuestionDetail({route, navigation}) {
    ...
    function validateComment() {
        ...
        if (isValid) {
            addCommentToQuestion(questionId, content).then((v) => {
                getQuestionDetail(questionId, null, null).then((r) =>
    ...
    const callbackAnswer = () => {
        getQuestionDetail(questionId, null, null).then((r) => setUserAnswer(r.data.answers.items));
    };
    const handleAnswer = () => {
    ...
    const sort = (sortByType) => {
        if (sortByType === "Score (DESC)") {
            getQuestionDetail(questionId, "DESC", null).then((r) => setUserAnswer(r.data.answers.items));
        } else if (sortByType === "Score (ASC)") {
            getQuestionDetail(questionId, "ASC", null).then((r) => setUserAnswer(r.data.answers.items));
        } else if (sortByType === "Date created (DESC)") {
            getQuestionDetail(questionId, null, "DESC").then((r) => setUserAnswer(r.data.answers.items));
        } else if (sortByType === "Date created (ASC)") {
            getQuestionDetail(questionId, null, "ASC").then((r) => setUserAnswer(r.data.answers.items));
        }
            ...
            <View style={styles.container}>
                ...
                <View style={styles.content}>
                    ...
                    <View style={styles.containerContent}>
                        <VoteComponent votes={votes} size={"4x"} onPressUp={() => handleVote("LIKED")}
                                       onPressDown={() => handleVote("DISLIKED")} status={votedStatus}/>
                        <View style={styles.containerCon}>
                            ...
                            />
                            {statusClose !== "CLOSE" && <View style={styles.containerComment}>
