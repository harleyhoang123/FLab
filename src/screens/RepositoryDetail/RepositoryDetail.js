...
function RepositoryDetail({ route, navigation }) {
  ...
  const deleteAFileOrFolder = (id, type, parentFolderId) => {
    deleteFolderOrFile(parentFolderId, id, type, navigation)
      .then((v) =>
        getListFolderDetail(parentFolderId, navigation)
          .then((r) => {
  ...
  const ItemFile = ({
    ...
    <View style={styles.table}>
      ...
      <View
        style={[styles.column, { alignItems: "flex-start", paddingLeft: 10 }]}
      >
        <View
          style={{
            flexDirection: "row",
        ...
        >
          ...
          >
            <MaterialCommunityIcons
              ...
              color="#3f444a"
            ></MaterialCommunityIcons>
            <Text style={{ color: "blue", marginLeft: 10 }}>{folderName}</Text>
          </View>
        </TouchableOpacity>
      </View>
      <View
