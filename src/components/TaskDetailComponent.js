...
export default function TaskDetailComponent({
    ...
    const [descriptionDetail, setDescriptionDetail] = useState(taskDetail.description);
    ...
    const renderTextField = () => {
        if (add) {
            return (
                <View>
                    <TextField style={{margin: 0, width: "100%", height: 40}} text={subTaskName}
                               onChangeText={subTaskName => setSubTaskName(subTaskName)}
                               onSubmitEditing={() => addSubTask(taskDetail.taskId, memberId, subTaskName)}/>
                    <View style={styles.rowDetail}>
                        <Buttons text={"Create"} onPressTo={() => addSubTask(taskDetail.taskId, memberId, subTaskName)}
                                 style={styles.button}/>
                        <Buttons text={"Cancel"} onPressTo={() => setAdd(!add)} style={styles.button}/>
                    </View>
                </View>
            )
        }
    ...
    const addSubTask = (taskId, memberId, subTaskName) => {
        createSubTask(projectId, taskId, memberId, subTaskName).then(() => getListSubTask(taskId).then(r => setListSubTask(r.data.subTasks)));
        setAdd(!add)
        setSubTaskName('')
    }
                    ...
                    <View style={styles.modalProfileView}>
                        ...
                        <View style={{alignItems: "flex-end", flexDirection: "row"}}>
                            <Buttons text={"Update"} style={{marginRight: 40}} onPressTo={() => {
                                ...
                                updateATask(projectId, taskDetail.taskId, taskNameUpdate, statusUpdate, descriptionUpdate, assigneeUpdate, labelUpdate, estimateUpdate, reporterUpdate);
                                setModalVisible(!modalVisible);
                            }}/>
            ...
            <View style={styles.wrapper}>
                ...
                <View style={{borderWidth: 1, borderRadius: 5}}>
                    {listSubTask?.map((item) => (
                        <SubTaskComponent key={item.subTaskId} subTaskId={item.subTaskId} status={item.status}
                                          estimate={item.estimate} subTaskName={item.subTaskName}
                                          assignee={item.assignee} callbackSubTaskDetail={callbackSubTaskDetail}/>
                    ))}
                    {renderTextField()}
                </View>
