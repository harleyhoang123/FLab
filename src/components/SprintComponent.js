...
function SprintComponent({
  ...
  const callBackGetListTask = () => {
    getListTask(sprintId, navigation)
      .then((r) => {
  ...
  const deleteASprint = (projectId, sprintId) => {
    deleteSprint(projectId, sprintId, navigation)
      .then(() => {
        callBackGetListSprint();
  ...
  const addTask = (sprintId, memberId, taskName) => {
    createTask(projectId, sprintId, memberId, taskName, navigation)
      .then(() =>
        getListTask(sprintId, navigation)
          .then((r) => {
            setListTask(r.data.tasks);
  ...
  const updateASprint = (
    ...
    updateSprint(
      ...
      .then((r) =>
        getSprintDetail(sprintId, navigation)
          .then((r) => {
  ...
  const updateStatusOfSprint = (sprintId, memberId, status) => {
    if (status === "NOT_START") {
      updateStatusSprint(projectId, sprintId, memberId, "STARTING", navigation)
        .then((r) =>
          getSprintDetail(sprintId, navigation)
            .then((r) => {
      ...
      updateStatusSprint(projectId, sprintId, memberId, "COMPLETED", navigation)
        .then((r) =>
          getSprintDetail(sprintId, navigation)
            .then((r) => {
              setStatusDetail(r.data.status);
    ...
    if (visible) {
      return (
        <View>
          ...
          {role !== "MEMBER" && <View>{renderTextField(isTextField)}</View>}
        </View>
      );
    }
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.containerContent}>
        ...
        <Buttons
          ...
          onPressTo={() => {
            role !== "MEMBER"
              ? updateStatusOfSprint(sprintId, memberId, statusDetail)
